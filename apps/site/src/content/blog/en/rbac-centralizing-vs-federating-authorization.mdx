...
The first idea was to build an **RBAC Core**. Every application would query this service before allowing an action. Follow the scenario: each step reveals where that centralization starts to weigh on the system.

<ScrollyFlow client:idle scenario={rbacCentralizedScenario} locale="en" />
...
<ScrollyFlow client:idle scenario={rbacFederatedScenario} locale="en" />
