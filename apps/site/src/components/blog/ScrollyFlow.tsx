...
export default function ScrollyFlow({ scenario: inputScenario, locale }: Props) {
  ...
  return (
    <section className="scrolly-flow" aria-labelledby={`${scenario.id}-title`} style={{ margin: "3rem auto", maxWidth: 1152 }}>
      ...
      <style>{`.scrolly-flow { box-sizing: border-box; width: min(72rem, calc(100vw - 3rem)); margin-left: 50% !important; transform: translateX(-50%); } .scrolly-flow-layout > * { min-width: 0; } html[data-reading-theme-active="light"] .scrolly-flow { --scrolly-surface: #fff; --scrolly-text: #111; --scrolly-node: #0a0a0a; --scrolly-node-text: #fff; --scrolly-muted: #6b7280; --scrolly-border: #d1d5db; } html[data-reading-theme-active="dark"] .scrolly-flow { --scrolly-surface: #161616; --scrolly-text: #f3f4f6; --scrolly-node: #242424; --scrolly-node-text: #fff; --scrolly-muted: #9ca3af; --scrolly-border: #3f3f46; } @media (max-width: 960px) { .scrolly-flow-layout { grid-template-columns: minmax(0, 1fr) !important; } .scrolly-flow-canvas:not(.is-mobile-exploring) { position: sticky !important; top: 6rem !important; height: clamp(280px, 52vw, 380px) !important; order: -1; } } @media (max-width: 720px) { .scrolly-flow-canvas:not(.is-mobile-exploring) { height: 280px !important; } } @media (prefers-reduced-motion: reduce) { .react-flow *, .react-flow__edge-path { transition: none !important; animation: none !important; } }`}</style>
    </section>
  );
}
